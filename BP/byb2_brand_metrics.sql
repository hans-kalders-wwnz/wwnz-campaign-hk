CREATE OR REPLACE TABLE gcp-wow-cd-email-app-test.campaign_excellence.byb2_brand_metrics as (
-- Brand metrics
...
SELECT b.*,
boosted_sales/points_cost as scr,
new_to_brand_redeemers,
reactivated_redeemers,
repeat_redeemers,
brand_shoppers,
non_boosters,
retained_repeat_shoppers,
retained_reactivated_shoppers,
retained_new_to_brand_shoppers
FROM brand b
LEFT JOIN new_to_brand_shoppers n ON n.brand = b.brand
LEFT JOIN reactivated_shoppers c ON c.brand = b.brand
LEFT JOIN repeat_shoppers t ON t.brand = b.brand
LEFT JOIN shopped_brand s ON s.brand = b.brand
LEFT JOIN did_not_boost nb ON nb.brand = b.brand
LEFT JOIN retained_repeat rr ON rr.brand = b.brand
LEFT JOIN retained_reactivated rc ON rc.brand = b.brand
LEFT JOIN retained_new_to_brand rn ON rn.brand = b.brand
ORDER BY brand
)
